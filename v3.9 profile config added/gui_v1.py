...
if __name__ == "__main__":
    ...
    class GUIConfig:
        def __init__(self) -> None:
            ...
            self.enable_auto_timbre: bool = False
            self.auto_timbre_threshold_hz: float = 150.0
            self.auto_timbre_strength: float = 0.5
            self.auto_timbre_minimum: float = 0.0
            self.auto_timbre_attack_ms: float = 100.0
            self.auto_timbre_release_ms: float = 200.0
    ...
    class GUI:
        ...
        def get_default_settings(self, input_devices, output_devices):
            return {
                ...
                "enable_auto_timbre": False, "auto_timbre_threshold_hz": 150.0, "auto_timbre_strength": 0.5,
                "auto_timbre_minimum": 0.0, "auto_timbre_attack_ms": 100.0, "auto_timbre_release_ms": 200.0,
        ...
        def launcher(self):
            ...
            enable_auto_timbre = data.get("enable_auto_timbre", False)
            ...
            self.realtime_param_keys = [
                ...
                "enable_auto_timbre", "auto_timbre_threshold_hz", "auto_timbre_strength", "auto_timbre_minimum", "auto_timbre_attack_ms", "auto_timbre_release_ms",
            ...
            auto_timbre_settings_layout = [
                [sg.Text("Threshold (Hz)"), sg.Slider(range=(50, 500), key="auto_timbre_threshold_hz", resolution=1, orientation="h", default_value=data.get("auto_timbre_threshold_hz", 150.0), expand_x=True, enable_events=True)],
                [sg.Text("Strength"), sg.Slider(range=(0.0, 1.0), key="auto_timbre_strength", resolution=0.01, orientation="h", default_value=data.get("auto_timbre_strength", 0.5), expand_x=True, enable_events=True)],
                [sg.Text("Minimum Timbre"), sg.Slider(range=(0.0, 2.0), key="auto_timbre_minimum", resolution=0.01, orientation="h", default_value=data.get("auto_timbre_minimum", 0.0), expand_x=True, enable_events=True)],
                [sg.Text("Attack (ms)"), sg.Slider(range=(10, 500), key="auto_timbre_attack_ms", resolution=10, orientation="h", default_value=data.get("auto_timbre_attack_ms", 100.0), expand_x=True, enable_events=True)],
                [sg.Text("Release (ms)"), sg.Slider(range=(10, 1000), key="auto_timbre_release_ms", resolution=10, orientation="h", default_value=data.get("auto_timbre_release_ms", 200.0), expand_x=True, enable_events=True)]
            ]
            formant_timbre_frame = sg.Frame("Formant & Timbre", [
                ...
                [sg.Text("Timbre"), sg.Slider(range=(0.0, 2.0), key="timbre", resolution=0.01, orientation="h", default_value=data.get("timbre", 1.0), expand_x=True, enable_events=True)],
                [sg.Checkbox("Enable Advanced Auto-Timbre", key="enable_auto_timbre", default=enable_auto_timbre, enable_events=True)],
                [sg.pin(sg.Column(auto_timbre_settings_layout, key='auto_timbre_column', visible=enable_auto_timbre))]
            ], expand_x=True)
            ...
            effects_col_layout = [[formant_timbre_frame], [dynamic_proximity_frame], [low_freq_dampening_frame], [saturation_frame], [discord_effects_frame], [cave_effect_frame], [reverb_frame], [phone_effect_frame], [sg.Frame('', [[]], size=(10, 800), border_width=0, pad=(0,0))]]
        ...
        def update_visibility(self, key, value):
            ...
            elif key == "enable_auto_timbre": self.window["auto_timbre_column"].update(visible=value)
