# gui_v1.py (FINAL, WITH REAL-TIME FILE VOLUME)
import os, sys
    ...
    class GUI:
        ...
        def _get_values_for_saving(self, values):
            ...
            keys_to_remove = [f'eq_gain_{hz}hz_db' for hz in self.eq_freq_bands] + \
                             ...
                             ["split_pitch_crossover_display", "-EQ_PRESET_COMBO-", 'm2f', 'f2m', 'default', '-INPUT_EQ_PRESET_COMBO-']
        ...
        def launcher(self):
            ...
            # --- NEXUS MODIFICATION: Add new slider key to the real-time list ---
        ...
        def event_handler(self):
            while True:
                ...
                if event == '-SAVE_PROFILE-': self._save_profile(values, values['-PROFILE_NAME-'])
                    ...
                    if self.audio_processor:
                        self.audio_processor.stop_file()
                        self.window['play_file_inference'].update(disabled=False if values.get("input_audio_path") else True)
                        self.window['play_file_direct'].update(disabled=False if values.get("input_audio_path") else True)
