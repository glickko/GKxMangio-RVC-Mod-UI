# rvc_for_realtime.py (COMPLETE AND FIXED)
...
class RVC:
    ...
    def pre_allocate_effects(self):
        self.reverb_effect, self.saturation_effect, self.cave_reverb_effect, self.cave_delay_effect = None, None, None, None
        self.dyn_prox_bass_shelf, self.dyn_prox_treble_shelf, self.dyn_prox_reverb = None, None, None
        self.equalizer, self.input_equalizer = None, None
        if pedalboard is not None:
            ...
            self.equalizer = pedalboard.Pedalboard([pedalboard.PeakFilter(cutoff_frequency_hz=hz, q=1.41, gain_db=0.0) for hz in self.eq_freq_bands])
            self.input_equalizer = pedalboard.Pedalboard([pedalboard.PeakFilter(cutoff_frequency_hz=hz, q=1.41, gain_db=0.0) for hz in self.eq_freq_bands])
    ...
    def _update_effect_params(self, key):
        ...
        if "eq_gains_db" in key: self._update_all_eq_params()
        if "reverb" in key and self.reverb_effect:
            self.reverb_effect.room_size, self.reverb_effect.damping = self.reverb_room_size, self.reverb_damping
            self.reverb_effect.wet_level, self.reverb_effect.dry_level = self.reverb_wet_level, self.reverb_dry_level
        if "saturation" in key and self.saturation_effect: self.saturation_effect.drive_db = self.saturation_drive_db
        if "cave" in key:
            if self.cave_reverb_effect:
                self.cave_reverb_effect.wet_level, self.cave_reverb_effect.dry_level = self.cave_mix, 1.0 - self.cave_mix
            if self.cave_delay_effect:
                self.cave_delay_effect.delay_seconds, self.cave_delay_effect.feedback, self.cave_delay_effect.mix = self.cave_delay_time / 1000.0, self.cave_feedback, self.cave_mix
        if "dynamic_proximity_room_size" in key and self.dyn_prox_reverb: self.dyn_prox_reverb.room_size = self.dynamic_proximity_room_size
    ...
    def _apply_auto_pitch_correction(self, f0, base_f0_up_key, auto_pitch_settings):
        ...
        if use_split and average_f0_hz < split_hz:
            current_strength, current_max_adjustment, self.effects_diag['pitch_range'] = low_strength, low_max, 'Low'
        elif use_split:
            current_strength, current_max_adjustment, self.effects_diag['pitch_range'] = high_strength, high_max, 'High'
        self.effects_diag['ap_strength'], self.effects_diag['ap_max_adj'] = current_strength, current_max_adjustment
    ...
    def _calculate_brightness(self, audio_chunk):
        try:
            spectrum = np.abs(np.fft.rfft(audio_chunk))
            freqs = np.fft.rfftfreq(len(audio_chunk), 1 / self.tgt_sr)
            mid_energy = np.sum(spectrum[(freqs >= 1000) & (freqs < 3000)])
            high_energy = np.sum(spectrum[(freqs >= 3000) & (freqs < 8000)])
            return high_energy / (mid_energy + 1e-8)
        except: return 0

    def _apply_dynamic_timbre(self, indata, settings):
        (enable, attack_ms, release_ms, low_cross, high_cross, 
         low_target, mid_target, high_target, low_curve, high_curve,
         enable_bright, bright_thresh, min_f0_bright) = settings
        
        if not enable:
            self.last_adjusted_timbre = self.timbre
            return self.timbre

        if self.smoothed_f0 < 1:
            self.last_adjusted_timbre = mid_target
            return mid_target

        target_timbre = mid_target
        
        # F0-based progress
        f0_progress = 0.0
        if self.smoothed_f0 > high_cross:
            f0_distance = np.clip((self.smoothed_f0 - high_cross) / 500, 0, 1) # 500hz is transition range
            f0_progress = f0_distance ** high_curve

        # Brightness-based progress
        bright_progress = 0.0
        if enable_bright and self.smoothed_f0 > min_f0_bright:
            brightness = self._calculate_brightness(indata)
            if brightness > bright_thresh:
                brightness_overshoot = brightness - bright_thresh
                # Scale progress: assume an overshoot of 2.0 is "max" progress
                bright_progress = np.clip(brightness_overshoot / 2.0, 0, 1)

        # Low band calculation
        if self.smoothed_f0 < low_cross: 
            transition_range = low_cross - 80 
            distance = np.clip((low_cross - self.smoothed_f0) / max(1, transition_range), 0, 1)
            progress = distance ** low_curve
            target_timbre = mid_target + (low_target - mid_target) * progress
        else: # Mid and High band
            progress = max(f0_progress, bright_progress)
            target_timbre = mid_target + (high_target - mid_target) * progress

        attack = attack_ms / (attack_ms + self.block_time * 1000)
        release = release_ms / (release_ms + self.block_time * 1000)
        stability = attack if target_timbre < self.last_adjusted_timbre else release
        
        smoothed_timbre = self.last_adjusted_timbre * stability + target_timbre * (1 - stability)
        self.last_adjusted_timbre = smoothed_timbre
        return smoothed_timbre
    
    def _apply_dynamic_formant(self, settings):
        (enable, low_target, mid_target, high_target, low_cross, high_cross, low_curve, high_curve) = settings
        if not enable:
            return self.formant_shift
        if self.smoothed_f0 < 1:
            return mid_target
        
        target_formant = mid_target
        if self.smoothed_f0 < low_cross:
            transition_range = low_cross - 80
            distance = np.clip((low_cross - self.smoothed_f0) / max(1, transition_range), 0, 1)
            progress = distance ** low_curve
            target_formant = mid_target + (low_target - mid_target) * progress
        elif self.smoothed_f0 > high_cross:
            transition_range = 500
            distance = np.clip((self.smoothed_f0 - high_cross) / transition_range, 0, 1)
            progress = distance ** high_curve
            target_formant = mid_target + (high_target - mid_target) * progress
        return target_formant
    ...
    def _apply_effects(self, audio_np: np.ndarray, rms_level: float, effects_settings):
        (
            ...
            enable_sat, sat_hz, enable_cave, use_reverb, enable_eq
        ) = effects_settings
        ...
        if enable_eq and self.equalizer:
            audio_np = self.equalizer(audio_np, self.tgt_sr)
    ...
    def get_f0(self, x, f0_up_key, rmvpe_threshold, auto_pitch_settings):
        ...
        self.last_avg_hz = np.mean(f0[f0 > 0]) if np.any(f0 > 0) else 0.0
        
        if self.smoothed_f0 == 0.0: self.smoothed_f0 = self.last_avg_hz
        else: self.smoothed_f0 = self.smoothed_f0 * self.f0_smoothing_factor + self.last_avg_hz * (1 - self.f0_smoothing_factor)
    ...
    def get_diagnostics_text(self, latency_ms):
        ...
        if self.last_updated_param_info:
            ...
            if ttime() - update_time < 1.5: diag_text += f"Applied: {param_key} | "
            else: self.last_updated_param_info = None
        ...
        with self.param_lock:
            ...
            if self.voice_profile in ["Male to Female", "Female to Male"] and self.enable_dynamic_timbre: 
                diag_text += f"DynTimbre: {self.last_adjusted_timbre:.2f} | "
            else:
                diag_text += f"Timbre: {self.timbre:.2f} | "

            if self.auto_pitch_correction and self.voice_profile in ["Male to Female", "Female to Male"]:
    ...
    def infer(self, feats: torch.Tensor, indata: np.ndarray, rate1, rate2, cache_pitch, cache_pitchf, rms_level: float, crossfade_frame: int) -> np.ndarray:
        with self.param_lock:
            pitch_val, index_rate_val, voice_profile_val = self.pitch, self.index_rate, self.voice_profile
            ...
            dynamic_timbre_settings = (self.enable_dynamic_timbre, self.dyn_timbre_attack_ms, self.dyn_timbre_release_ms, self.low_pitch_crossover, self.high_pitch_crossover, self.low_timbre_target, self.mid_timbre_target, self.high_timbre_target, self.low_curve_factor, self.high_curve_factor, self.enable_brightness_sensor, self.brightness_threshold, self.min_f0_for_brightness)
            dynamic_formant_settings = (self.enable_dynamic_formant, self.low_formant_target, self.mid_formant_target, self.high_formant_target, self.low_pitch_crossover, self.high_pitch_crossover, self.low_formant_curve_factor, self.high_formant_curve_factor)
            effects_settings = (self.enable_low_freq_dampening, self.low_freq_dampening_threshold_hz, self.low_freq_dampening_level_db, self.enable_dynamic_proximity, self.dynamic_proximity_strength, self.enable_discord_effects, self.discord_quality, self.discord_proximity, self.discord_noise, self.enable_phone_effect, self.enable_saturation_effect, self.saturation_threshold_hz, self.enable_cave_effect, self.use_reverb, self.enable_equalizer)
        ...
        if self.if_f0 == 1:
            pitch, pitchf = self.get_f0(indata, pitch_val, 0.01, auto_pitch_settings)
            
            if voice_profile_val in ["Male to Female", "Female to Male"]:
                final_timbre_val = self._apply_dynamic_timbre(indata, dynamic_timbre_settings)
                final_formant_val = self._apply_dynamic_formant(dynamic_formant_settings)
            else:
                final_timbre_val = self.timbre
                final_formant_val = self.formant_shift

            timbre_ramp = np.ones(pitchf.shape[0])
            ramp_len_in_pitch_frames = crossfade_frame // self.window
            ramp_len = min(pitchf.shape[0], ramp_len_in_pitch_frames)
            
            if ramp_len > 0:
                timbre_ramp[:ramp_len] = np.linspace(self.last_used_timbre_for_sola, final_timbre_val, ramp_len)
            
            timbre_ramp[ramp_len:] = final_timbre_val
            
            pitchf *= final_formant_val
            pitchf += (timbre_ramp - 1.0) * self.sr / self.window
            
            self.last_used_timbre_for_sola = final_timbre_val
