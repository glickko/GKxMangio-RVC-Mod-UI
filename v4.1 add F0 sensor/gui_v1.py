...
if __name__ == "__main__":
    ...
    class GUIConfig:
        def __init__(self) -> None:
            ...
            self.timbre: float = 1.0 # Serves as default timbre and mid-band target
    ...
    class GUI:
        ...
        def get_default_settings(self, input_devices, output_devices):
            return {
                ...
                "high_timbre_target": 1.0, "low_curve_factor": 1.0, "high_curve_factor": 1.0,
        ...
        def _get_values_for_saving(self, values):
            """Helper function to gather all current values into a savable dictionary."""
            ...
            # Remove transient or composite keys that shouldn't be saved directly
        ...
        def _load_profile(self, profile_name):
            """Loads settings from a named profile and applies them."""
            profile_path = os.path.join(self.profiles_folder, f"{profile_name}.json")
            try:
        ...
        def _update_profile_list(self):
            """Scans the profiles folder and updates the dropdown list."""
            try:
        ...
        def _apply_settings_to_gui_and_backend(self, data):
            """Applies a dictionary of settings to the GUI and running audio processor."""
            # Update basic elements first
            for key, value in data.items():
                if key in self.window.key_dict:
                    ...
                    if isinstance(element, (sg.Slider, sg.Input, sg.Checkbox)):
                        ...
                        # For device combos, we just set the value. The list is dynamic.
            ...
            # Update visibility of conditional UI sections
            for vis_key in self.visibility_param_keys:
                if vis_key in data:
                    self.update_visibility(vis_key, data[vis_key])
            
            # Queue updates to the running audio processor
            if self.audio_processor:
                # Create a comprehensive dictionary of all parameters to update
        ...
        def launcher(self):
            ...
            self.realtime_param_keys = [
                "threhold", "pitch", "index_rate", "input_volume", "input_formant_shift", "input_timbre", "output_volume", "I_noise_reduce", "O_noise_reduce", "formant_shift", "timbre", "bypass_vc",
                "enable_dynamic_timbre", "dyn_timbre_attack_ms", "dyn_timbre_release_ms", "low_pitch_crossover", "high_pitch_crossover", "low_timbre_target", "high_timbre_target", "low_curve_factor", "high_curve_factor",
            ...
            self.visibility_param_keys = ["use_shout_dampening", "use_split_pitch_correction", "use_reverb", "enable_discord_effects", "enable_saturation_effect", "enable_cave_effect", "enable_low_freq_dampening", "enable_dynamic_proximity", "auto_pitch_correction", "enable_phone_effect", "enable_dynamic_timbre", "enable_equalizer", "enable_input_equalizer"]
            
            self.main_reset_keys = ["threhold", "pitch", "index_rate", "input_volume", "input_formant_shift", "input_timbre", "output_volume", "output_volume_normalization", "bypass_vc", "auto_pitch_correction", "default", "m2f", "f2m", "f2f", "m2m", "enable_input_equalizer", "input_eq_gains_db"]
            self.effects_reset_keys = ["formant_shift", "timbre", "enable_dynamic_timbre", "dyn_timbre_attack_ms", "dyn_timbre_release_ms", "low_pitch_crossover", "high_pitch_crossover", "low_timbre_target", "high_timbre_target", "low_curve_factor", "high_curve_factor", "enable_brightness_sensor", "brightness_threshold", "min_f0_for_brightness", "enable_low_freq_dampening", "low_freq_dampening_threshold_hz", "low_freq_dampening_level_db", "enable_saturation_effect", "saturation_threshold_hz", "saturation_drive_db", "enable_dynamic_proximity", "dynamic_proximity_strength", "dynamic_proximity_room_size", "enable_discord_effects", "discord_proximity", "discord_noise", "discord_quality", "enable_cave_effect", "cave_delay_time", "cave_feedback", "cave_mix", "use_reverb", "reverb_room_size", "reverb_damping", "reverb_wet_level", "reverb_dry_level", "enable_phone_effect"]
            ...
            input_equalizer_controls_layout = sg.Column([
                [
                    sg.Button("Flatten", key='-INPUT_EQ_FLATTEN-', size=(11,1)),
                    sg.Button("Invert", key='-INPUT_EQ_INVERT-', size=(11,1)),
                ],[
                    sg.Button("Bass Boost", key='-INPUT_EQ_BASS_BOOST-', size=(11,1)),
                    sg.Button("Treble Boost", key='-INPUT_EQ_TREBLE_BOOST-', size=(11,1)),
                ],[
                    sg.Button("Bass Cut", key='-INPUT_EQ_BASS_CUT-', size=(11,1)),
                    sg.Button("Treble Cut", key='-INPUT_EQ_TREBLE_CUT-', size=(11,1)),
                ]
            ], pad=(10,10))

            input_equalizer_frame = sg.Frame("Input Equalizer", [
                [sg.Checkbox("Enable Input EQ", key="enable_input_equalizer", default=enable_input_equalizer, enable_events=True)],
                [sg.pin(sg.Column([
            ...
            main_tab_layout = [
                ...
                [sg.Text("Input Formant"), sg.Slider(range=(0.0, 2.0), key="input_formant_shift", resolution=0.01, orientation="h", default_value=data.get("input_formant_shift", 1.0), expand_x=True, tooltip="Shift formants of the input audio. Approximated by pitch shifting.", enable_events=True)],
                [sg.Text("Input Timbre"), sg.Slider(range=(0.0, 2.0), key="input_timbre", resolution=0.01, orientation="h", default_value=data.get("input_timbre", 1.0), expand_x=True, tooltip="Adjust timbre (brightness) of the input audio.", enable_events=True)],
                [
                    sg.Text("Output Volume (dB)"), 
                    sg.Slider(
                        range=(-60, 12), key="output_volume", resolution=1, orientation="h", 
                        default_value=data.get("output_volume", 0.0), expand_x=True, 
                        tooltip="Final output volume.", enable_events=True, 
                        disabled=output_volume_normalization_enabled
                    ),
                    sg.Checkbox(
                        "Normalize (YouTube)", key="output_volume_normalization", 
                        default=output_volume_normalization_enabled, 
                        tooltip="Locks volume to a level suitable for YouTube (-14 LUFS target).", 
                        enable_events=True
                    )
                ],
                [sg.Checkbox("Bypass Voice Changer", key="bypass_vc", default=bypass_vc_enabled, enable_events=True, tooltip="Pass microphone audio directly to output.")],
                ...
                [sg.pin(sg.Column([[sg.Checkbox("Auto Pitch Correction", key="auto_pitch_correction", default=auto_pitch_correction_enabled, tooltip="Auto-adjust pitch to profile.", enable_events=True)]], key="auto_pitch_checkbox_column", visible=current_profile in ["Male to Female", "Female to Male"]))],
            ...
            model_layout = [
                ...
                [sg.Input(default_text=data.get("pth_path", ""), key="pth_path", expand_x=True), sg.FileBrowse("Select .pth file", initial_folder=os.path.join(os.getcwd(), "weights"), file_types=((". pth"),))],
                [sg.pin(sg.Column([[sg.Input(default_text=data.get("index_path", ""), key="index_path", expand_x=True), sg.FileBrowse("Select .index file", initial_folder=os.path.join(os.getcwd(), "logs"), file_types=((". index"),)) ]], key='index_path_row', visible=use_index_file))]
            ]
            ...
            # --- DYNAMIC TIMBRE & BRIGHTNESS SENSOR LAYOUT ---
            brightness_sensor_layout = sg.Frame("Brightness Sensor", [
                [sg.Checkbox("Enable Brightness Sensor", key="enable_brightness_sensor", default=data.get("enable_brightness_sensor", False), enable_events=True, tooltip="Triggers high band based on spectral brightness, not just F0")],
                [sg.Text("Brightness Threshold"), sg.Slider(range=(0.1, 5.0), key="brightness_threshold", resolution=0.05, orientation="h", default_value=data.get("brightness_threshold", 0.8), expand_x=True, enable_events=True)],
                [sg.Text("Minimum F0 (Hz)"), sg.Slider(range=(80, 300), key="min_f0_for_brightness", resolution=1, orientation="h", default_value=data.get("min_f0_for_brightness", 170.0), expand_x=True, enable_events=True)],
            ], expand_x=True)

            dynamic_timbre_settings_layout = [
                [sg.Text("Attack (ms)"), sg.Slider(range=(10, 500), key="dyn_timbre_attack_ms", resolution=10, orientation="h", default_value=data.get("dyn_timbre_attack_ms", 100.0), expand_x=True, enable_events=True)],
                [sg.Text("Release (ms)"), sg.Slider(range=(10, 1000), key="dyn_timbre_release_ms", resolution=10, orientation="h", default_value=data.get("dyn_timbre_release_ms", 200.0), expand_x=True, enable_events=True)],
                [sg.HSep()],
                [sg.Text("Low/Mid Crossover (Hz)"), sg.Slider(range=(50, 300), key="low_pitch_crossover", resolution=1, orientation="h", default_value=data.get("low_pitch_crossover", 150.0), expand_x=True, enable_events=True)],
                [sg.Text("Mid/High Crossover (Hz)"), sg.Slider(range=(150, 800), key="high_pitch_crossover", resolution=1, orientation="h", default_value=data.get("high_pitch_crossover", 400.0), expand_x=True, enable_events=True)],
                [sg.HSep()],
                [sg.Frame("Low Pitch Band", [
                    [sg.Text("Target Timbre"), sg.Slider(range=(0.0, 2.0), key="low_timbre_target", resolution=0.01, orientation="h", default_value=data.get("low_timbre_target", 1.0), expand_x=True, enable_events=True)],
                    [sg.Text("Curve Factor"), sg.Slider(range=(0.1, 3.0), key="low_curve_factor", resolution=0.01, orientation="h", default_value=data.get("low_curve_factor", 1.0), expand_x=True, enable_events=True)],
                ], expand_x=True)],
                [sg.Frame("High Pitch Band", [
                    [sg.Text("Target Timbre"), sg.Slider(range=(0.0, 2.0), key="high_timbre_target", resolution=0.01, orientation="h", default_value=data.get("high_timbre_target", 1.0), expand_x=True, enable_events=True)],
                    [sg.Text("Curve Factor"), sg.Slider(range=(0.1, 3.0), key="high_curve_factor", resolution=0.01, orientation="h", default_value=data.get("high_curve_factor", 1.0), expand_x=True, enable_events=True)],
                ], expand_x=True)],
                [sg.HSep()],
                [brightness_sensor_layout],
            ]
            dynamic_timbre_frame = sg.Frame("Dynamic Timbre Shaper", [
                [sg.Checkbox("Enable Dynamic Timbre", key="enable_dynamic_timbre", default=enable_dynamic_timbre, enable_events=True)],
                [sg.pin(sg.Column(dynamic_timbre_settings_layout, key='dynamic_timbre_column', visible=enable_dynamic_timbre))]
            ], expand_x=True)

            formant_timbre_frame = sg.Frame("Global Formant & Timbre", [
                [sg.Text("Formant Shift"), sg.Slider(range=(0.0, 2.0), key="formant_shift", resolution=0.01, orientation="h", default_value=data.get("formant_shift", 1.0), expand_x=True, enable_events=True)], 
                [sg.pin(sg.Column([[sg.Text("Default Timbre"), sg.Slider(range=(0.0, 2.0), key="timbre", resolution=0.01, orientation="h", default_value=data.get("timbre", 1.0), expand_x=True, enable_events=True)]], key='default_timbre_row', visible=not enable_dynamic_timbre))]
            ], expand_x=True)
            ...
            equalizer_controls_layout = sg.Column([
                [sg.Checkbox("Enable EQ", key="enable_equalizer", default=enable_equalizer, enable_events=True)],
                [
                    sg.Text("Preset"),
                    sg.Combo(eq_preset_list, key='-EQ_PRESET_COMBO-', readonly=True, expand_x=True),
                    sg.Button("Apply", key='-EQ_APPLY_PRESET-')
                ]
            ], pad=(10,10))

            equalizer_tab_layout = [
                [sg.Push(), sg.Button("Reset", key="-RESET_EQUALIZER-", size=(8,1))],
            ...
            profiles_tab_layout = [
                ...
                [sg.HSep()],
                [sg.Text("Profile Name", size=(12,1)), sg.Input(key='-PROFILE_NAME-', expand_x=True)],
                [sg.Button("Save Current Profile", key='-SAVE_PROFILE-', expand_x=True)],
                [sg.HSep()],
                [sg.Text("Load Profile", size=(12,1)), sg.Combo([], key='-PROFILE_LIST-', expand_x=True, readonly=True)],
                [
                    sg.Button("Load Selected", key='-LOAD_PROFILE-'),
                    sg.Button("Refresh List", key='-REFRESH_PROFILES-'),
                    sg.Button("Delete Selected", key='-DELETE_PROFILE-', button_color=('white', '#8B0000'))
                ]
            ]
        ...
        def update_visibility(self, key, value):
            ...
            if key == "use_shout_dampening": self.window['shout_dampening_slider_row'].update(visible=value)
                ...
                self.window["default_timbre_row"].update(visible=not value)
        ...
        def event_handler(self):
            while True:
                ...
                if isinstance(event, str) and event.startswith("-RESET_"):
                    ...
                    if event == "-RESET_EQUALIZER-": self.handle_reset(self.equalizer_reset_keys)
                    elif event == "-RESET_MAIN-": self.handle_reset(self.main_reset_keys)
                    elif event in key_map: self.handle_reset(key_map[event])
                    continue
                
                if event == '-SAVE_PROFILE-':
                    self._save_profile(values, values['-PROFILE_NAME-'])
                    continue
                if event == '-LOAD_PROFILE-':
                    if values['-PROFILE_LIST-']:
                        self._load_profile(values['-PROFILE_LIST-'])
                    else:
                        sg.popup("No profile selected.", title="Info")
                    continue
                if event == '-DELETE_PROFILE-':
                    if values['-PROFILE_LIST-']:
                        self._delete_profile(values['-PROFILE_LIST-'])
                    else:
                        sg.popup("No profile selected to delete.", title="Info")
                    continue
                if event == '-REFRESH_PROFILES-':
                    self._update_profile_list()
                    continue

                if event == "output_volume_normalization":
                    ...
                    if is_normalized:
                        self.window["output_volume"].update(value=new_volume)
                    if self.audio_processor:
                        self.audio_processor.queue_parameter_update("output_volume", new_volume)
                    continue
                
                if event == '-EQ_APPLY_PRESET-':
                    prefix = "" # Output EQ
                    selected_preset = values['-EQ_PRESET_COMBO-']
                    if not selected_preset: continue
                    
                    preset_gains = None
                    if selected_preset == "Flatten": preset_gains = {hz: 0.0 for hz in self.eq_freq_bands}
                    elif selected_preset == "Invert": preset_gains = {hz: -values.get(f'eq_gain_{hz}hz_db', 0.0) for hz in self.eq_freq_bands}
                    elif selected_preset == "Bass Boost": preset_gains = {31: 9.0, 62: 6.0, 125: 3.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: 0.0, 8000: 0.0, 16000: 0.0}
                    elif selected_preset == "Treble Boost": preset_gains = {31: 0.0, 62: 0.0, 125: 0.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: 3.0, 8000: 6.0, 16000: 9.0}
                    elif selected_preset == "Bass Cut": preset_gains = {31: -9.0, 62: -6.0, 125: -3.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: 0.0, 8000: 0.0, 16000: 0.0}
                    elif selected_preset == "Treble Cut": preset_gains = {31: 0.0, 62: 0.0, 125: 0.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: -3.0, 8000: -6.0, 16000: -9.0}
                    elif selected_preset == "Studio": preset_gains = {31: 1.0, 62: 2.0, 125: 1.0, 250: -1.0, 500: -0.5, 1000: 0.0, 2000: 1.0, 4000: 2.0, 8000: 3.0, 16000: 2.0}
                    elif selected_preset == "HQ Phone": preset_gains = {31: -12.0, 62: -9.0, 125: -6.0, 250: -3.0, 500: 0.0, 1000: 3.0, 2000: 6.0, 4000: 3.0, 8000: -3.0, 16000: -9.0}
                    elif selected_preset == "LQ Phone": preset_gains = {31: -12.0, 62: -12.0, 125: -9.0, 250: -6.0, 500: 3.0, 1000: 6.0, 2000: 9.0, 4000: 0.0, 8000: -9.0, 16000: -12.0}

                    if preset_gains:
                        for hz, gain in preset_gains.items():
                            slider_key = f'{prefix}eq_gain_{hz}hz_db'
                            label_key = f'{prefix}eq_gain_label_{hz}hz'
                            self.window[slider_key].update(gain)
                            self.window[label_key].update(f"{gain:.0f} dB")
                            values[slider_key] = gain
                        if self.audio_processor:
                            self.audio_processor.queue_parameter_update("eq_gains_db", preset_gains)
                    continue

                if isinstance(event, str) and event.startswith('-INPUT_EQ_'):
                    prefix = "input_"
                    clean_event = event.strip('-').replace('INPUT_', '')
                    
                    preset_gains = None
                    if clean_event == "EQ_FLATTEN": preset_gains = {hz: 0.0 for hz in self.eq_freq_bands}
                    elif clean_event == "EQ_INVERT": preset_gains = {hz: -values.get(f'{prefix}eq_gain_{hz}hz_db', 0.0) for hz in self.eq_freq_bands}
                    elif clean_event == "EQ_BASS_BOOST": preset_gains = {31: 9.0, 62: 6.0, 125: 3.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: 0.0, 8000: 0.0, 16000: 0.0}
                    elif clean_event == "EQ_TREBLE_BOOST": preset_gains = {31: 0.0, 62: 0.0, 125: 0.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: 3.0, 8000: 6.0, 16000: 9.0}
                    elif clean_event == "EQ_BASS_CUT": preset_gains = {31: -9.0, 62: -6.0, 125: -3.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: 0.0, 8000: 0.0, 16000: 0.0}
                    elif clean_event == "EQ_TREBLE_CUT": preset_gains = {31: 0.0, 62: 0.0, 125: 0.0, 250: 0.0, 500: 0.0, 1000: 0.0, 2000: 0.0, 4000: -3.0, 8000: -6.0, 16000: -9.0}
                    ...
                    if preset_gains:
                        for hz, gain in preset_gains.items():
                            ...
                            values[slider_key] = gain
                        if self.audio_processor:
                            self.audio_processor.queue_parameter_update("input_eq_gains_db", preset_gains)
                    continue
                
                if isinstance(event, str) and (event.startswith('eq_gain_') or event.startswith('input_eq_gain_')):
                    is_input = event.startswith("input_")
                    prefix = "input_" if is_input else ""
                    try:
                        hz_str = event.split('_')[3 if is_input else 2].replace('hz', '')
                        hz = int(hz_str)
                        gain = values[event]
                        self.window[f'{prefix}eq_gain_label_{hz}hz'].update(f"{gain:.0f} dB")
                        if self.audio_processor:
                            current_gains = {h: values.get(f'{prefix}eq_gain_{h}hz_db', 0.0) for h in self.eq_freq_bands}
                            self.audio_processor.queue_parameter_update(f'{prefix}eq_gains_db', current_gains)
                    except (ValueError, KeyError, IndexError) as e: 
                        print(f"Error processing EQ slider event: {event}, Error: {e}")
                    continue
                ...
                if event == "refresh_devices":
                    ...
                    if self.audio_processor and values["input_audio_path"]: self.audio_processor.request_load_file(values["input_audio_path"])
