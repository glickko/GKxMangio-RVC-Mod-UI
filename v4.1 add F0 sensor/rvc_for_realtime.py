...
class RVC:
    ...
    def pre_allocate_effects(self):
        ...
        if pedalboard is not None:
            ...
            output_eq_filters = [pedalboard.PeakFilter(cutoff_frequency_hz=hz, q=1.41, gain_db=0.0) for hz in self.eq_freq_bands]
            self.equalizer = pedalboard.Pedalboard(output_eq_filters)
            
            input_eq_filters = [pedalboard.PeakFilter(cutoff_frequency_hz=hz, q=1.41, gain_db=0.0) for hz in self.eq_freq_bands]
            self.input_equalizer = pedalboard.Pedalboard(input_eq_filters)
    ...
    def _update_all_eq_params(self):
        if self.equalizer:
            for i, hz in enumerate(self.eq_freq_bands):
                try:
                    gain = self.eq_gains_db.get(hz, 0.0)
                    self.equalizer[i].gain_db = gain
                except Exception as e:
                    print(f"Error updating Output EQ band {hz}Hz: {e}")
        if self.input_equalizer:
            for i, hz in enumerate(self.eq_freq_bands):
                try:
                    gain = self.input_eq_gains_db.get(hz, 0.0)
                    self.input_equalizer[i].gain_db = gain
                except Exception as e:
                    print(f"Error updating Input EQ band {hz}Hz: {e}")
    ...
    def _apply_dynamic_timbre(self, indata, settings):
        ...
        # Brightness Sensor Logic
        if enable_bright and self.last_avg_hz > min_f0_bright:
            brightness = self._calculate_brightness(indata)
            if brightness > bright_thresh:
                is_bright = True

        # Determine pitch band and calculate target timbre
        if self.last_avg_hz < low_cross: # Low Band
            transition_range = low_cross - 80 
            distance = np.clip((low_cross - self.last_avg_hz) / max(1, transition_range), 0, 1)
            progress = distance ** low_curve
            target_timbre = mid_target + (low_target - mid_target) * progress

        elif self.last_avg_hz > high_cross or is_bright: # High Band (triggered by F0 OR brightness)
            if is_bright and not self.last_avg_hz > high_cross:
                # If triggered by brightness, assume full progress to make it responsive
                progress = 1.0
            else: # Triggered by F0
                transition_range = 500
                distance = np.clip((self.last_avg_hz - high_cross) / transition_range, 0, 1)
                progress = distance ** high_curve
            target_timbre = mid_target + (high_target - mid_target) * progress
        
        # Smoothing (Attack/Release)
        attack_stability = attack_ms / (attack_ms + self.block_time * 1000)
        release_stability = release_ms / (release_ms + self.block_time * 1000)
        stability = attack_stability if target_timbre < self.last_adjusted_timbre else release_stability
    ...
    def get_diagnostics_text(self, latency_ms):
        ...
        with self.param_lock:
            ...
            if self.enable_dynamic_timbre: diag_text += f"DynTimbre: {self.last_adjusted_timbre:.2f} | "
    ...
    def infer(self, feats: torch.Tensor, indata: np.ndarray, rate1, rate2, cache_pitch, cache_pitchf, rms_level: float, crossfade_frame: int) -> np.ndarray:
        with self.param_lock:
            pitch_val, index_rate_val, formant_val, voice_profile_val = self.pitch, self.index_rate, self.formant_shift, self.voice_profile
            ...
            dynamic_timbre_settings = (self.enable_dynamic_timbre, self.dyn_timbre_attack_ms, self.dyn_timbre_release_ms, self.low_pitch_crossover, self.high_pitch_crossover, self.low_timbre_target, self.timbre, self.high_timbre_target, self.low_curve_factor, self.high_curve_factor, self.enable_brightness_sensor, self.brightness_threshold, self.min_f0_for_brightness)
