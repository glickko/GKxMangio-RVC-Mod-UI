# gui_v1.py (COMPLETE AND MODIFIED FOR REAL-TIME UPDATES)
import os, sys
...
if __name__ == "__main__":
    ...
    class GUI:
        ...
        def print_device_diagnostics(self):
            """Queries and prints a detailed list of all audio devices to the console."""
            try:
                sd._terminate()
                sd._initialize()
                ...
                print("\n" + "="*80)
                print("|| NEXUS ON-DEMAND SCANNER: DETECTED AUDIO DEVICES".center(80) + "||")
                print("="*80)
        ...
        def __init__(self) -> None:
            ...
            ### MODIFICATION: Added a dictionary to store the last known values of sliders for real-time updates.
        ...
        def launcher(self):
            ...
            ### MODIFICATION: This list now includes every slider we want to update in real-time.
            self.realtime_slider_keys = [
                "threhold", "pitch", "index_rate", "input_volume", "input_formant_shift", "input_timbre", "output_volume", "timbre",
            ...
            for hz in self.eq_freq_bands:
                ...
                band_col = sg.Column([
                    ...
                    ### MODIFICATION: Changed range from (12, -12) to (-12, 12) for intuitive control (up=boost, down=cut).
            ...
            # In gui_v1.py, inside the launcher method
        ...
        ### MODIFICATION: New method to handle polling of slider values.
        def _handle_realtime_slider_updates(self, values):
            """
            Checks for changes in slider values and sends updates to the audio processor.
            This runs on every iteration of the event loop.
            """
            ...
            for key in self.realtime_slider_keys:
                ...
                if current_value != last_value:
                    ...
                    # For EQ sliders, also update the dB label in real-time
                    if 'eq_gain' in key:
                        ...
                        # Update the entire dictionary for the backend
                        ...
                        self.audio_processor.queue_parameter_update(eq_type, gains_dict)
                         ...
                         # For all other sliders, send the single value update
                        self.audio_processor.queue_parameter_update(key, current_value)

        def event_handler(self):
            while True:
                ### MODIFICATION: Added a timeout to window.read() to enable continuous polling.
                # A timeout of 20ms means the loop runs ~50 times per second.
                event, values = self.window.read(timeout=20)
                
                ### MODIFICATION: Call the new update handler at the start of every loop iteration.
                self._handle_realtime_slider_updates(values)
                
                if event == sg.TIMEOUT_KEY:
                    continue # Nothing to do on a timeout event, just continue the loop
                ...
                ### MODIFICATION: This block now primarily handles non-slider events (checkboxes, combos, buttons).
                ### Slider events are caught by the polling method, but this remains for fallback and checkboxes.
                elif event in self.realtime_param_keys:
                    if self.audio_processor:
                        # Handle non-slider real-time parameters like checkboxes
                    ...
                    # Update visibility for checkboxes that control other elements
                    if event in self.visibility_param_keys: self.update_visibility(event, values[event])


                # In gui_v1.py, inside the event_handler method, add this new 'elif' block
                ...
                # --- NEXUS MODIFICATION: Add event handler for the new button ---
                elif event == "-CHECK_DEVICES_CMD-":
                    self.print_device_diagnostics()
                # --- END MODIFICATION ---
