...
import time, traceback, sys, threading, queue, math
try:
    import pedalboard
except ImportError:
    pedalboard = None
...
class AudioProcessor:
    ...
    def _soundinput(self):
        try:
            ...
            # --- NEXUS MODIFICATION START: Simplified On-Start Info ---
            print("\n" + "-"*60)
            input_device_info = sd.query_devices(input_device_idx)['name']
            output_device_info = sd.query_devices(output_device_idx)['name']
            print(f"Attempting to use Input: '{input_device_info}' (Index: {input_device_idx})")
            print(f"Attempting to use Output: '{output_device_info}' (Index: {output_device_idx})")
            print(f"Sample Rate: {self.config.device_samplerate} Hz")
            print("-"*60)
            # --- NEXUS MODIFICATION END ---
            
            # Pre-flight Check from previous step (still important)
            sd.check_input_settings(device=input_device_idx, samplerate=self.config.device_samplerate)
            sd.check_output_settings(device=output_device_idx, samplerate=self.config.device_samplerate)
            ...
            with sd.InputStream(
                ...
                while self.flag_vc:
                    indata, overflowed = stream_in.read(self.block_frame_device)
                    if overflowed:
                        print("Warning: Input stream has overflowed!")
                    
                    output_chunk = self._process_chunk(indata[:, 0]).reshape(-1, 1)
                    
                    stream_out.write(output_chunk)

        except sd.PortAudioError as pae:
            error_message = f"PortAudio Error: {pae.args[0]}. This is likely an unsupported sample rate for the selected audio device. Please use the 'Check Devices in CMD' button to verify your device's capabilities and select a supported sample rate."
            print("!!! FAILED TO START AUDIO STREAM (INVALID SETTINGS) !!!")
            print("="*60)
            print(error_message)
            print("="*60)
            self.window.write_event_value('-STREAM_ERROR-', error_message)
    ...
    def _mic_callback(self, indata: np.ndarray, outdata: np.ndarray, frames, times, status):
        # This method is no longer used by the microphone input thread, 
        # but we can leave it in case it's referenced elsewhere or for future use.
        # It will not be called during microphone processing with the new _soundinput method.
        pass
    ...
    def _process_chunk(self, indata_mono: np.ndarray) -> np.ndarray:
        ...
        # --- TAMBAHKAN BLOK DEBUGGING DI SINI ---
        max_val = np.max(np.abs(indata_mono))
        print(f"Processing chunk... Max input volume: {max_val:.4f}")
        if max_val == 0.0:
            print("WARNING: Input audio is completely silent!")
        # --- AKHIR BLOK DEBUGGING ---
        
        input_volume, output_volume = self.config.input_volume, self.config.output_volume
        input_formant = self.config.input_formant_shift
        input_timbre = self.config.input_timbre
