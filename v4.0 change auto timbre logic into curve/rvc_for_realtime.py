...
class RVC:
    ...
    def _apply_dynamic_timbre(self, settings):
        (enable, attack_ms, release_ms, low_cross, high_cross, 
         low_target, mid_target, high_target, low_curve, high_curve) = settings
        
        if not enable or self.last_avg_hz < 1:
        ...
        if self.last_avg_hz < low_cross: # Low Band
            # Transition from Mid to Low
            transition_range = low_cross - 80 # e.g. 150hz -> 70hz, a reasonable low-end range
            distance = np.clip((low_cross - self.last_avg_hz) / transition_range, 0, 1)
            progress = distance ** low_curve
            target_timbre = mid_target + (low_target - mid_target) * progress

        elif self.last_avg_hz > high_cross: # High Band
            # Transition from Mid to High
            transition_range = 500 # Assume a 500Hz range for high-end transition
    ...
    def infer(self, feats: torch.Tensor, indata: np.ndarray, rate1, rate2, cache_pitch, cache_pitchf, rms_level: float, crossfade_frame: int) -> np.ndarray:
        with self.param_lock:
            ...
            dynamic_timbre_settings = (self.enable_dynamic_timbre, self.dyn_timbre_attack_ms, self.dyn_timbre_release_ms, self.low_pitch_crossover, self.high_pitch_crossover, self.low_timbre_target, self.mid_timbre_target, self.high_timbre_target, self.low_curve_factor, self.high_curve_factor)
        ...
        if self.if_f0 == 1:
            ...
            if voice_profile_val != "Default":
                final_timbre_val = self._apply_dynamic_timbre(dynamic_timbre_settings)
