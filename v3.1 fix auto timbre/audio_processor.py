...
class AudioProcessor:
    ...
    def _process_chunk(self, indata_mono: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # --- Corrected SOLA with Normalized Cross-Correlation ---
            ...
            # Normalize signals for scale-invariant correlation
            sola_search_area_norm = sola_search_area_np - np.mean(sola_search_area_np)
            sola_buffer_norm = sola_buffer_np - np.mean(sola_buffer_np)
            
            search_std = np.std(sola_search_area_norm)
            buffer_std = np.std(sola_buffer_norm)

            if search_std > 1e-8 and buffer_std > 1e-8:
                sola_search_area_norm /= search_std
                sola_buffer_norm /= buffer_std
                correlation = np.correlate(sola_search_area_norm, sola_buffer_norm, mode='valid')
                
                max_corr_value = np.max(correlation)
                sola_offset = np.argmax(correlation)

                # Confidence threshold for the correlation.
                CORRELATION_CONFIDENCE_THRESHOLD = 0.4
                if max_corr_value < CORRELATION_CONFIDENCE_THRESHOLD:
                    sola_offset = 0 # Fallback to direct overlap if match is poor
            else:
                # If one of the signals is silent, just do a direct overlap.
                sola_offset = 0
