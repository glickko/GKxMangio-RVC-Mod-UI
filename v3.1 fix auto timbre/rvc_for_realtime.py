...
class RVC:
    ...
    def _apply_effects(self, audio_np: np.ndarray, rms_level: float, effects_settings):
        (
            ...
            enable_sat, sat_hz, enable_cave, use_reverb, use_auto_timbre, auto_timbre_shout_hz,
            auto_timbre_strength
        ) = effects_settings
        ...
        if use_auto_timbre and self.voice_profile != "Default" and pedalboard is not None:
            target_timbre = self.timbre
            if self.last_avg_hz > auto_timbre_shout_hz:
                pitch_excess_semitones = 12 * np.log2(self.last_avg_hz / auto_timbre_shout_hz)
                reduction = pitch_excess_semitones * auto_timbre_strength * 0.01
                target_timbre = self.timbre - reduction
            
            current_auto_timbre = np.clip(target_timbre, 0.0, 2.0)
            self.effects_diag['auto_timbre_val'] = current_auto_timbre
            
            semitone_shift = (current_auto_timbre - self.timbre) * 4.0
            if abs(semitone_shift) > 0.01:
                # Create a new stateless effect instance to avoid transition artifacts
                auto_timbre_effect = pedalboard.PitchShift(semitones=semitone_shift)
                audio_np = auto_timbre_effect(audio_np, self.tgt_sr)
    ...
    def get_diagnostics_text(self, latency_ms):
        ...
        with self.param_lock:
            ...
            if self.use_auto_timbre and self.voice_profile != "Default": 
                diag_text += f"AutoTimbre: {self.effects_diag.get('auto_timbre_val', self.timbre):.2f} | "
    ...
    def infer(self, feats: torch.Tensor, indata: np.ndarray, rate1, rate2, cache_pitch, cache_pitchf, rms_level: float) -> np.ndarray:
        with self.param_lock:
            ...
            effects_settings = (self.enable_low_freq_dampening, self.low_freq_dampening_threshold_hz, self.low_freq_dampening_level_db, self.enable_dynamic_proximity, self.dynamic_proximity_strength, self.enable_discord_effects, self.discord_quality, self.discord_proximity, self.discord_noise, self.enable_phone_effect, self.enable_saturation_effect, self.saturation_threshold_hz, self.enable_cave_effect, self.use_reverb, self.use_auto_timbre, self.auto_timbre_shout_hz, self.auto_timbre_strength)
