...
if __name__ == "__main__":
    ...
    class GUI:
        ...
        def save(self, values):
            ...
            values['eq_gains_db'] = {hz: values.get(f'eq_gain_{hz}hz_db', 0.0) for hz in self.eq_freq_bands}
            values['input_eq_gains_db'] = {hz: values.get(f'input_eq_gain_{hz}hz_db', 0.0) for hz in self.eq_freq_bands}
            
            keys_to_remove = [f'eq_gain_{hz}hz_db' for hz in self.eq_freq_bands] + [f'input_eq_gain_{hz}hz_db' for hz in self.eq_freq_bands] + ["split_pitch_crossover_display", "-EQ_PRESET_COMBO-"]
            temp_values = {k: v for k, v in values.items() if k not in keys_to_remove}
