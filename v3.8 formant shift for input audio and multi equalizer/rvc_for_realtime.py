...
class RVC:
    def __init__(self, gui_config) -> None:
        ...
        try:
            # The RVC models are loaded here. This is necessary even if bypass_vc is initially true,
            # in case the user toggles it off later during runtime. The bypass is handled in the audio processing loop.
    ...
    def _apply_auto_timbre(self, base_timbre, settings):
        enable, threshold_hz, strength, attack_ms, release_ms, minimum_timbre = settings
        if not enable or self.last_avg_hz == 0:
            self.last_adjusted_timbre = base_timbre
            return base_timbre

        target_timbre = base_timbre
        if self.last_avg_hz > threshold_hz:
            reduction = (self.last_avg_hz - threshold_hz) * (strength / 500.0)
            target_timbre = base_timbre - reduction
        
        target_timbre = np.clip(target_timbre, minimum_timbre, 2.0)
    ...
    def get_diagnostics_text(self, latency_ms):
        ...
        with self.param_lock:
            ...
            if self.enable_auto_timbre: diag_text += f"Timbre: {self.last_adjusted_timbre:.2f} | "
    ...
    def infer(self, feats: torch.Tensor, indata: np.ndarray, rate1, rate2, cache_pitch, cache_pitchf, rms_level: float, crossfade_frame: int) -> np.ndarray:
        with self.param_lock:
            ...
            auto_timbre_settings = (self.enable_auto_timbre, self.auto_timbre_threshold_hz, self.auto_timbre_strength, self.auto_timbre_attack_ms, self.auto_timbre_release_ms, self.auto_timbre_minimum)
        ...
        if self.if_f0 == 1:
            ...
            if voice_profile_val != "Default":
                final_timbre_val = self._apply_auto_timbre(timbre_val, auto_timbre_settings)
