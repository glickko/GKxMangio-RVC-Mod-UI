# gui_v1.py (VONOX-LOGIC INTEGRATED)
import os, sys
...
if __name__ == "__main__":
    import numpy as np
    import traceback, re, shutil, subprocess
    import json, queue
    import PySimpleGUI as sg
    import sounddevice as sd
    import torch, time, threading, webbrowser
    ...
    class GUIConfig:
        def __init__(self) -> None:
            ...
            self.file_input_volume_db: float = 0.0
            ...
            self.enable_cpu_affinity: bool = True
            self.cpu_cores_to_use: int = 4
            self.enable_upscaler: bool = False
    ...
    class GUI:
        def __init__(self) -> None:
            ...
            self.is_ffmpeg_available = self.check_ffmpeg()
            self.launcher()

        def check_ffmpeg(self):
            if shutil.which("ffmpeg"):
                print("FFmpeg found. Hybrid file input mode is enabled.")
                return True
            else:
                print("WARNING: FFmpeg not found in system PATH. File input will be disabled.")
                return False

        def print_device_diagnostics(self):
            try:
                sd._terminate(); sd._initialize()
                devices, hostapis = sd.query_devices(), sd.query_hostapis()
                print("\n" + "="*80 + "\n||" + " NEXUS ON-DEMAND SCANNER: DETECTED AUDIO DEVICES".center(76) + "||\n" + "="*80)
                for i, device in enumerate(devices):
                    device_api_info = hostapis[device['hostapi']]['name']
                    print(
                        f"  Device {i}: {device['name']}\n"
                        f"    - Index: {device['index']}, API: {device_api_info}\n"
                        f"    - Inputs: {int(device['max_input_channels'])}, Outputs: {int(device['max_output_channels'])}\n"
                        f"    - Default Sample Rate: {device['default_samplerate']:.0f} Hz\n"
                    )
                print("="*80)
                sg.popup("Device list printed to the command line console.", title="Device Scan Complete")
            except Exception as e:
                sg.popup(f"Failed to scan devices: {e}", title="Error")
        ...
        def get_default_settings(self, input_devices, output_devices):
            return {
                ...
                "f0method": "rmvpe", "precision": "FP16", "hop_length": 128,
                "device_samplerate": 48000, "sola_search_ms": 10.0,
                "I_noise_reduce": False, "O_noise_reduce": False,
                "input_volume": 1.0, "file_input_volume_db": 0.0, "input_formant_shift": 1.0, "input_timbre": 1.0, "output_volume": 0.0, "output_volume_normalization": False, "bypass_vc": False,
                ...
                "input_audio_path": "", "loop_audio_file": False,
                ...
                "enable_cpu_affinity": True, "cpu_cores_to_use": 4, "enable_upscaler": False
            }

        def load(self):
            input_devices, output_devices, _, _ = self.get_devices(update=True)
            
            smart_precision_default = "FP16"
            if torch.cuda.is_available():
                major, _ = torch.cuda.get_device_capability()
                if major >= 8:
                    smart_precision_default = "TF32"

            try:
                with open("values1.json", "r") as j:
                    data = json.load(j)
            except (FileNotFoundError, json.JSONDecodeError):
                print("Settings file not found or corrupt, creating new one.")
                data = self.get_default_settings(input_devices, output_devices)
                if smart_precision_default == "TF32":
                    print("TF32-compatible GPU detected. Setting default precision to TF32.")
                    data["precision"] = "TF32"
                with open("values1.json", "w") as j:
                    json.dump(data, j, indent=4)
                return data

            if "is_half" in data and "precision" not in data:
                data["precision"] = "FP16" if data["is_half"] else "FP32"
                del data["is_half"]
            
            if "precision" not in data and smart_precision_default == "TF32":
                 print("TF32-compatible GPU detected. Setting default precision to TF32 for existing config.")
                 data["precision"] = "TF32"
        ...
        def _get_values_for_saving(self, values):
            ...
            if values.get('fp16'):
                save_values['precision'] = "FP16"
            elif values.get('tf32'):
                save_values['precision'] = "TF32"
            else:
                save_values['precision'] = "FP32"
            ...
            keys_to_remove = [f'eq_gain_{hz}hz_db' for hz in self.eq_freq_bands] + \
                             ...
                             ["split_pitch_crossover_display", "-EQ_PRESET_COMBO-", 'm2f', 'f2m', 'default', '-INPUT_EQ_PRESET_COMBO-', 'fp16', 'fp32', 'tf32']
        ...
        def _apply_settings_to_gui_and_backend(self, data):
            ...
            self.window['fp16'].update(precision == "FP16")
            self.window['fp32'].update(precision == "FP32")
            self.window['tf32'].update(precision == "TF32")
        ...
        def _apply_eq_preset(self, preset_name, values, prefix=""):
            ...
            elif preset_name == "Bass Boost": new_gains.update({31: 6, 62: 4, 125: 2})
            elif preset_name == "Treble Boost": new_gains.update({4000: 2, 8000: 4, 16000: 6})
            elif preset_name == "Bass Cut": new_gains.update({31: -6, 62: -4, 125: -2})
            elif preset_name == "Treble Cut": new_gains.update({4000: -2, 8000: -4, 16000: -6})
            elif preset_name == "Studio": new_gains.update({31: 3, 62: 2, 125: 1, 500: -1, 1000: -2, 2000: -1, 8000: 2, 16000: 4})
            elif preset_name == "HQ Phone": new_gains.update({31: -12, 62: -8, 125: -4, 8000: -4, 16000: -8})
            elif preset_name == "LQ Phone": new_gains.update({31: -12, 62: -10, 125: -8, 250: -4, 4000: -4, 8000: -8, 16000: -12})
        ...
        def launcher(self):
            ...
            self.realtime_slider_keys = [
                "threhold", "pitch", "index_rate", "input_volume", "file_input_volume_db", "input_formant_shift", "input_timbre", "output_volume", "timbre",
                "f0_smoothing_factor", "pitch_stability", "auto_pitch_strength", "auto_pitch_max_adjustment", "shout_dampening_strength",
                "low_pitch_strength", "low_pitch_max_adjustment", "high_pitch_strength", "high_pitch_max_adjustment",
                "formant_shift", "formant_attack_ms", "formant_release_ms", "low_formant_target", "mid_formant_target", "high_formant_target", "low_formant_curve_factor", "high_formant_curve_factor",
                "dyn_timbre_attack_ms", "dyn_timbre_release_ms", "low_pitch_crossover", "high_pitch_crossover", "low_timbre_target", "mid_timbre_target", "high_timbre_target", "low_curve_factor", "high_curve_factor",
                "brightness_threshold", "min_f0_for_brightness",
                "reverb_room_size", "reverb_damping", "reverb_wet_level", "reverb_dry_level",
                "discord_proximity", "discord_noise", "discord_quality",
                "saturation_threshold_hz", "saturation_drive_db",
                "cave_delay_time", "cave_feedback", "cave_mix",
                "low_freq_dampening_threshold_hz", "low_freq_dampening_level_db",
                "dynamic_proximity_strength", "dynamic_proximity_room_size",
            ] + [f'eq_gain_{hz}hz_db' for hz in self.eq_freq_bands] + [f'input_eq_gain_{hz}hz_db' for hz in self.eq_freq_bands]

            self.realtime_param_keys = [
                "threhold", "pitch", "index_rate", "input_volume", "file_input_volume_db", "input_formant_shift", "input_timbre", "output_volume", "I_noise_reduce", "O_noise_reduce", "timbre", "bypass_vc",
            ...
            self.main_reset_keys = ["threhold", "pitch", "index_rate", "input_volume", "file_input_volume_db", "input_formant_shift", "input_timbre", "output_volume", "output_volume_normalization", "bypass_vc", "default", "m2f", "f2m", "enable_input_equalizer", "input_eq_gains_db"]
            ...
            self.performance_reset_keys = ["I_noise_reduce", "O_noise_reduce", "enable_cmd_diagnostics", "precision", "enable_cpu_affinity", "cpu_cores_to_use", "enable_upscaler"]
            ...
            file_input_disabled = not self.is_ffmpeg_available
            file_input_tooltip = "Load any audio/video file to play through the voice changer.\nRequires FFmpeg to be installed on your system." if not file_input_disabled else "File input disabled: FFmpeg not found in your system's PATH."

            hybrid_input_layout = [
                [sg.Frame("Microphone Input", [[
                    sg.Text("Input Device"),
                    sg.Combo(input_devices, key="sg_input_device", default_value=data.get("sg_input_device", ""), expand_x=True, readonly=True, tooltip="Select your microphone.")
                ]], expand_x=True)],
                [sg.Frame("Hybrid File Input", [
                    [sg.Input(default_text=data.get("input_audio_path", ""), key="input_audio_path", expand_x=True, disabled=file_input_disabled, enable_events=True),
                     sg.FileBrowse("Browse Files", file_types=(("All Files", "*.*"),), disabled=file_input_disabled)],
                    [sg.Text("File Volume (dB)"), sg.Slider(range=(-60, 12), key="file_input_volume_db", resolution=1, orientation="h", default_value=data.get("file_input_volume_db", 0.0), expand_x=True, disabled=file_input_disabled, enable_events=True, tooltip="Adjusts the volume of the loaded file.")],
                ], expand_x=True, tooltip=file_input_tooltip)]
            ]

            devices_layout = [
                [sg.Text("Output Device"), sg.Combo(output_devices, key="sg_output_device", default_value=data.get("sg_output_device", ""), expand_x=True, tooltip="Select your speakers or virtual audio cable.", readonly=True, enable_events=True)],
                [
                    sg.Text("Device Sample Rate"), sg.Combo(samplerate_options, key="device_samplerate", default_value=data.get("device_samplerate", 48000), expand_x=True, tooltip="The sample rate for your audio devices. Must match your device settings.\n48000Hz is recommended."),
                    sg.Button("Refresh", key="refresh_devices", tooltip="Rescans your system for available audio devices."),
                    sg.Button("Check Devices in CMD", key="-CHECK_DEVICES_CMD-", tooltip="Prints a detailed list of all audio devices to the console.")
                ]
            ]

            model_devices_tab_layout = [[sg.Frame("Load Model", model_layout, expand_x=True)], [sg.Frame("Input Sources (Hybrid Mode)", hybrid_input_layout, expand_x=True)], [sg.Frame("Audio Devices", devices_layout, expand_x=True)]]
            ...
            performance_settings_layout = [
                ...
                [
                    sg.Text("Precision"),
                    sg.Radio("FP16", "precision", key="fp16", default=data.get("precision") == "FP16", tooltip="Half-precision floating point. Faster, uses less VRAM, but may be less accurate."),
                    sg.Radio("FP32", "precision", key="fp32", default=data.get("precision") == "FP32", tooltip="Full-precision floating point. Slower, uses more VRAM, but is more accurate."),
                    sg.Radio("TF32", "precision", key="tf32", default=data.get("precision") == "TF32", tooltip="TensorFloat-32 for NVIDIA Ampere+ GPUs. FP32 range with FP16 speed. Requires model in FP32.")
                ],
                ...
                [sg.HSep()],
                [sg.Frame("Advanced Performance", [
                    [
                        sg.Checkbox("Enable CPU Affinity", key="enable_cpu_affinity", default=data.get("enable_cpu_affinity", True), tooltip="Binds the process to specific CPU cores to reduce stutter.\nRequires restart to apply."),
                        sg.Text("Cores to use:"),
                        sg.Input(key="cpu_cores_to_use", default_text=str(data.get("cpu_cores_to_use", 4)), size=(4,1), tooltip="Number of CPU cores to dedicate to audio processing.")
                    ],
                    [sg.Checkbox("Enable Output Upscaler", key="enable_upscaler", default=data.get("enable_upscaler", False), tooltip="Resamples the output audio to 2x the sample rate for potentially higher quality.\nMay increase CPU usage.", enable_events=True)]
                ], expand_x=True)],
            ...
            control_bar_layout = [
                ...
                sg.Text("|"),
                sg.Button("Play Inside Inference", key="play_file_inference", disabled=True, size=(16, 1)),
                sg.Button("Play Outside Inference", key="play_file_direct", disabled=True, size=(16, 1)),
                sg.Button("Stop File", key="stop_file", disabled=True, size=(8,1)),
                sg.Checkbox("Loop", key="loop_audio_file", default=data.get("loop_audio_file", False), disabled=True, enable_events=True, tooltip="Loop the audio file when it finishes."),
                sg.Push(),
        ...
        def handle_reset(self, keys_to_reset):
            for key in keys_to_reset:
                ...
                if key == "eq_gains_db" or key == "input_eq_gains_db":
                    ...
                    default_precision = self.default_settings.get("precision", "FP16")
                    self.window['fp16'].update(default_precision == "FP16")
                    self.window['fp32'].update(default_precision == "FP32")
                    self.window['tf32'].update(default_precision == "TF32")
                else:
        ...
        def update_visibility(self, key, value):
            ...
            elif key == "enable_dynamic_timbre": self.window["dynamic_timbre_column"].update(visible=value)
            elif key == "enable_dynamic_formant": self.window["dynamic_formant_column"].update(visible=value)
        ...
        def _handle_realtime_slider_updates(self, values):
            if not self.audio_processor or not self.audio_processor.flag_vc:
                return

            for key in self.realtime_slider_keys:
                ...
                if current_value != last_value:
                    ...
                    if 'eq_gain' in key:
                        prefix = "input_" if "input_" in key else ""
                        hz_part = key.replace(f'{prefix}eq_gain_', '').replace('hz_db', '')
                        self.window[f'{prefix}eq_gain_label_{hz_part}hz'].update(f"{current_value:.0f} dB")
                        eq_type = f'{prefix}eq_gains_db'
                        gains_dict = {hz: values.get(f'{prefix}eq_gain_{hz}hz_db', 0.0) for hz in self.eq_freq_bands}
                        self.audio_processor.queue_parameter_update(eq_type, gains_dict)
                    else:
                        self.audio_processor.queue_parameter_update(key, current_value)

        def event_handler(self):
            while True:
                event, values = self.window.read(timeout=20)

                self._handle_realtime_slider_updates(values)

                if event == sg.TIMEOUT_KEY:
                    if self.audio_processor and self.audio_processor.flag_vc:
                        is_playing = self.audio_processor.is_any_file_playing()
                        has_path = bool(self.audio_processor.file_audio_path)

                        self.window['play_file_inference'].update(disabled=is_playing or not has_path)
                        self.window['play_file_direct'].update(disabled=is_playing or not has_path)
                        self.window['stop_file'].update(disabled=not is_playing)
                    continue
                ...
                if event == '-SAVE_PROFILE-': self._save_profile(values, values['-PROFILE_NAME-'])
                ...
                elif event == '-CHECK_DEVICES_CMD-': self.print_device_diagnostics()
                elif event == '-EQ_APPLY_PRESET-': self._apply_eq_preset(values['-EQ_PRESET_COMBO-'], values, prefix="")
                elif event == '-INPUT_EQ_APPLY_PRESET-': self._apply_eq_preset(values['-INPUT_EQ_PRESET_COMBO-'], values, prefix="input_")

                elif event == "input_audio_path" and values["input_audio_path"]:
                    if self.audio_processor:
                        threading.Thread(target=self.audio_processor.request_load_file, args=(values["input_audio_path"],), daemon=True).start()
                    ...
                    if self.audio_processor:
                        if event not in self.realtime_slider_keys:
                            param_key = event
                    ...
                    if self.is_stopping:
                        sg.popup("Cannot start, previous session is still stopping. Please wait.")
                        continue
                    if self.set_values(values): self.start_vc()
                elif event == "stop_vc":
                    if not self.is_stopping:
                        self.save(values)
                        self.stop_vc()
                elif event == "play_file_inference":
                    if self.audio_processor: self.audio_processor.play_file_inference()
                elif event == "play_file_direct":
                    if self.audio_processor: self.audio_processor.play_file_direct()
                elif event == "stop_file":
                    if self.audio_processor:
                        threading.Thread(target=self.audio_processor.stop_file, daemon=True).start()
                        has_path = bool(values.get("input_audio_path"))
                        self.window['play_file_inference'].update(disabled=not has_path)
                        self.window['play_file_direct'].update(disabled=not has_path)
                    ...
                    self.window['play_file_inference'].update(disabled=True)
                    self.window['play_file_direct'].update(disabled=True)
                    self.window['stop_file'].update(disabled=True)
                    self.window['loop_audio_file'].update(disabled=True)
                    print("---STOPPED---")
        ...
        def stop_vc(self):
            if self.audio_processor:
                ...
                def stop_task():
                    ...
                    self.window.write_event_value('-STOP_COMPLETE-', '')
        ...
        def set_values(self, values):
            ...
            # Add new Vonovox settings
            self.config.enable_cpu_affinity = values["enable_cpu_affinity"]
            self.config.cpu_cores_to_use = int(values["cpu_cores_to_use"])
            self.config.enable_upscaler = values["enable_upscaler"]
        ...
        def start_vc(self):
            ...
            if self.is_stopping:
                ...
                return
            try:
                if self.config.precision == "TF32":
                    if torch.cuda.is_available() and torch.cuda.get_device_capability()[0] >= 8:
                        print("---TF32-ENABLED---"); torch.backends.cuda.matmul.allow_tf32 = True; torch.backends.cudnn.allow_tf32 = True
                    else:
                        print("---WARNING: TF32 not supported on this GPU, falling back to FP32.---"); self.config.precision = "FP32"; torch.backends.cuda.matmul.allow_tf32 = False; torch.backends.cudnn.allow_tf32 = False
                else:
                    print(f"---{self.config.precision} MODE---"); torch.backends.cuda.matmul.allow_tf32 = False; torch.backends.cudnn.allow_tf32 = False
                rvc = RVC(self.config)
                ...
                if self.config.input_audio_path and os.path.exists(self.config.input_audio_path):
                    self.audio_processor.request_load_file(self.config.input_audio_path)

                self.audio_processor.start()
                self.window["start_vc"].update(disabled=True)
                self.window["stop_vc"].update(disabled=False)
                if self.is_ffmpeg_available:
                    self.window['loop_audio_file'].update(disabled=False)
            except Exception as e:
        ...
        def get_devices(self, update: bool = True):
            ...
            desired_apis = ['MME', 'Windows WASAPI']
            filtered_devices = []

            for hostapi in hostapis:
                if hostapi["name"] in desired_apis:
                    for device_idx in hostapi["devices"]:
                        device = devices[device_idx]
                        device["hostapi_name"] = hostapi["name"]
                        filtered_devices.append(device)

            input_devices = [f"{d['name']} ({d['hostapi_name']})" for d in filtered_devices if d["max_input_channels"] > 0]
            output_devices = [f"{d['name']} ({d['hostapi_name']})" for d in filtered_devices if d["max_output_channels"] > 0]
            input_devices_indices = [d["index"] for d in filtered_devices if d["max_input_channels"] > 0]
            output_devices_indices = [d["index"] for d in filtered_devices if d["max_output_channels"] > 0]
