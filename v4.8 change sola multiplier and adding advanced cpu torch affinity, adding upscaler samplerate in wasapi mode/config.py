...
class Config:
    ...
    def device_config(self) -> tuple:
        if torch.cuda.is_available():
            ...
            major, _ = torch.cuda.get_device_capability(i_device)

            print("\n===== Hardware Support Analysis =====")
            print(f"Detected GPU: {self.gpu_name}")

            supported_precisions = ["FP32"]
            recommended_precision = "FP32"
            
            is_problematic_fp16 = any(card in self.gpu_name.upper() for card in ["16", "P40", "1060", "1070", "1080"]) and "V100" not in self.gpu_name.upper()
            
            if major >= 7 and not is_problematic_fp16:
                supported_precisions.append("FP16")
                recommended_precision = "FP16"
            elif is_problematic_fp16:
                 print("Note: This GPU has known issues with FP16, it will be disabled.")

            if major >= 8:
                supported_precisions.append("TF32")
                recommended_precision = "TF32"
            
            print(f"Supported Precisions: {', '.join(supported_precisions)}")
            print(f"Recommended Default Precision: {recommended_precision}")
            print("=======================================\n")
            
            self.is_half = not is_problematic_fp16
            ...
            if self.gpu_mem <= 4:
                try:
                    ...
                    print(f"Could not modify training script for low VRAM: {e}")

        elif self.has_mps():
            print("No supported Nvidia GPU found, using MPS instead. Precision: FP32")
            self.device = "mps"
            self.is_half = False
        else:
            print("No supported Nvidia GPU found, using CPU instead. Precision: FP32")
            self.device = "cpu"
            self.is_half = False
